...
import 'package:crudcursofirebase/model/curso.dart';
...
class CursoDao{
  ...
  Future<String> inserir(Curso curso) async{
    try {
      ...
      databaseReference.collection(COLECAO).add(curso.toMap());
      return ref.id;
    } catch (e) {
      return e.toString();
    }
  ...
  Future<String> alterar(Curso curso) async{
    try {
      await
      databaseReference.collection(COLECAO).doc(curso.id).update(curso.toMap());
  ...
  Future<List<Curso>> findAll() async {
    QuerySnapshot cursos = await
    databaseReference.collection("cursos").get();
